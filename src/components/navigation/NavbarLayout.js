  ...
  return (
    <nav className="bg-white  px-2 sm:px-4   dark:bg-secondary-main fixed w-full">
      <div className="container flex flex-wrap items-center justify-between mx-auto">
        ...
        <div className={`${navMobile && "hidden"} w-full md:block md:w-auto`} id="navbar-default">
          <ul className="flex flex-col p-4 mt-4  border-gray-100 rounded-lg md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium ">
            {pages.map((item, index) => (
              <li key={index + item.name}>
                <Link href={item.path} onClick={() => setNavMobile(true)} className="block py-2 pl-3 pr-4 rounded border-secondary-main text-white hover:bg-primary-main focus:bg-primary-main hover:text-secondary-main focus:text-secondary-main">{item.name}</Link>
              </li>
            ))}
          </ul>
        </div>
